...
make_preds_summ <- function(mat, sp = NULL, p1, p2 , xp1 = NULL, xp2 = NULL){
  # get mcmc matrix for non inxs
  m1 <- mat[, grep(paste0(p1,"\\[",sp), colnames(mat))]
  # get mcmc matrix for inxs
  m2 <- mat[,grep(paste0(p2, "\\[",sp), colnames(mat))]
  # calculating the occupancy rate at equilibrium
  without <- plogis(m1 %*% t(xp1))
  without <- apply(without, 2, quantile, probs = c(0.025,0.5,0.975))
  #
  with <- plogis(m1 %*% t(xp1) + m2 %*% t(xp2))
  with <- apply(with, 2, quantile, probs = c(0.025,0.5,0.975))
  #
  return(list(wo = without, w = with))
}

make_preds <- function(mat, sp = NULL, p1, p2 , xp1 = NULL, xp2 = NULL){
  # get mcmc matrix for non inxs
  m1 <- mat[, grep(paste0(p1,"\\[",sp), colnames(mat))]
  # get mcmc matrix for inxs
  m2 <- mat[,grep(paste0(p2, "\\[",sp), colnames(mat))]
  ...
  with <- plogis(m1 %*% t(xp1) + m2 %*% t(xp2))
  #
  return(data.frame(wo = c(without), w = c(with)))
}
